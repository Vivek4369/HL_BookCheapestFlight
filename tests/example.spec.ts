...
test.describe("Test to Book flight in MMT", () =>{
    ...
    test("test1", async ({})=>{
        ...
        //select flights 
        await page.locator("//a[@class='headerIcons makeFlex hrtlCenter column active']").click()
        
        //select flight from
        await page.locator("//label[@for='fromCity']").click()
        await page.locator('//input[@aria-controls="react-autowhatever-1"]').fill('delhi')
        await page.locator('//div[@class="makeFlex appendTop12  forHideundefined"]').click()

        //select fligh to
        await page.locator('//label[@for="toCity"]').click()
        await page.locator('//input[@aria-controls="react-autowhatever-1"]').fill('dehradun')
        await page.locator('//div[@class="makeFlex appendTop12  forHideundefined"]').click()


        const calendarField = page.locator('//div[@class="DayPicker"]');
        await expect(calendarField).toBeVisible();

        const dates = await page.locator('//div[@class="dateInnerCell"]').elementHandles()
